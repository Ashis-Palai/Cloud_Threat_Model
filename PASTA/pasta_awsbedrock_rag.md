...
| 15     | **Authentication & Authorization** | **Inconsistent Authentication Schemes on Single Route** <br> **Misconfiguration: Dual Auth on Same API Method** <br> **CWE-1220: Insufficient Granularity of Access Control** | **Design / Runtime** | **High** | This vulnerability occurs when a single API endpoint (e.g., POST /chat, GET /reports/export) is designed to accept both Cognito-based JWT tokens (for human users) and IAM SigV4-signed requests (for internal services), but without strict validation of which identity type is expected per request.Developers often fall into this trap during rapid iteration or migration phases, attempting to support both interactive users and service accounts through the same route or Lambda handler. While API Gateway only allows one authorizationType per method (e.g., AWS_IAM or COGNITO_USER_POOLS), this dual-auth behavior emerges when:<br>• Separate stages or domains are configured with different authorizers but route to the same backend integration.<br>• Resource policies allow execute-api:Invoke from IAM principals on routes primarily intended for JWT-authenticated users.
| 16     | **API Gateway / Request Transformation** | **Insecure VTL Mapping / Request Injection**<br>**CWE-1336: Improper Neutralization of Special Elements Used in a Template Engine** | **Design / Runtime** | **Critical** | This vulnerability arises when **Velocity Template Language (VTL)** mappings in API Gateway improperly blend **untrusted user input (e.g., headers, body fields)** into the **trusted request context** passed to downstream services (e.g., Lambda). <br><br>In many APIs, developers use VTL to pre-process requests before forwarding them to backend integrations. However, if templates **naively assign user-controlled values (like `X-User-Id` or `tenantId`) into sensitive context fields** (e.g., `context.authorizer`, `request.user_id`, `claims.tenant`), it creates a direct **injection primitive**.<br><br>**Exploit Flow:**<br>• An attacker crafts a request with a malicious header or body value (e.g., `X-User-Id: victim-123`).<br>• The VTL mapping template merges this into the request context as if it were a trusted claim:<br>`vtl<br>#set($context.requestOverride.user_id = $input.params('X-User-Id'))<br>`<br>• The Lambda function receives the request with a spoofed `user_id`, which may be used for authorization decisions, tenant lookups, or access control.<br>• If the backend trusts this `user_id` blindly (as many do), the attacker gains **cross-tenant or cross-user access**.<br>
| 17     | **Routing / HTTP Method Handling** | **ANY Method Misuse & Verb Tampering**<br>**CWE-650: Trusting HTTP Permission Methods on the Server Side** | **Design / Runtime** | **High** | This vulnerability occurs when **API Gateway routes** are configured using the `ANY` method (to accept all HTTP verbs), but the **backend handler (e.g., Lambda)** is only designed to safely process **specific verbs like `POST`**. All other methods (`GET`, `PUT`, `OPTIONS`, `TRACE`, etc.) are routed to the same function, which may not properly validate or handle them.<br><br>**Root Cause:** This typically happens out of convenience or default behavior — developers define a single `ANY` method on a route like `/resource`, and assume only `POST` or `GET` will be used. The Lambda then lacks method-specific logic and falls back to default cases for unknown methods, which may expose sensitive behavior.
| 18     | **API Gateway / Backend Integration** | **Integration Timeout Mismatch**<br>**CWE-770: Allocation of Resources Without Limits or Throttling** | **Design** | **High** | This issue arises when the **timeout value configured on the API Gateway integration** is **shorter than the timeout of the backend Lambda function**. For instance, API Gateway might timeout at **29 seconds**, while the associated Lambda is allowed to run for up to **60 seconds**. <br><br>**Why it happens:**<br>• Developers overlook timeout harmonization between API Gateway and Lambda.<br>• Defaults vary: API Gateway enforces shorter max timeouts than Lambda.<br>• There's often a lack of performance testing or cost modeling for long-running or cold-start-prone functions.<br>• No guardrails in deployment pipelines to enforce timeout alignment. |
| 19 | **API Gateway / Error Handling** | **Verbose Error Message Configuration**<br>**CWE-209: Information Exposure Through an Error Message** | Runtime | Medium | API Gateway and Lambda integrations often leak excessive detail in error responses — including backend function names, regions, execution ARNs, IAM denial messages, and even partial stack traces. These details surface via default integration settings or when error pass-through is left enabled.<br><br>Why it happens:<br>• Developers rely on default integration response behavior during development.<br>• Debug-friendly configurations (like pass-through or full error propagation) are left active in production.<br>• Custom Gateway Responses or error mapping templates are not defined to sanitize or standardize output.
| 20    | **API Gateway / Caching & Data Exposure** | **Sensitive Data Caching**<br>**CWE-524: Use of Cache Containing Sensitive Information** | **Design** | **High** | This misconfiguration occurs when **API Gateway response caching** is enabled for endpoints that return **user-specific or session-specific data**, but the **cache key is not properly partitioned** by user identity or session ID.<br><br>**Why it happens:**<br>• Developers enable caching to improve performance or reduce backend load, but fail to include **user-specific identifiers** (e.g., Cognito `user_id`, access token) in the cache key configuration.<br>• Cache keys are often derived from query parameters, path variables, or body content — which may collide across users if identical inputs are sent (e.g., same message like “hello”).<br>• Error responses may also be cached by default, leading to unexpected behavior when errors are reused across sessions or users.<br>• There is **no default enforcement in API Gateway** to separate cache space per user or role unless explicitly configured. |
| 21    | **API Gateway / CORS Configuration** | **Overly Permissive CORS Policy**<br>**CWE-346: Origin Validation Error** | **Runtime** | **High** | This vulnerability arises when the API Gateway (or an integrated backend) sets overly broad CORS headers — particularly `Access-Control-Allow-Origin: *` or wildcard origins that include attacker-controlled domains — **in combination with credentialed flows** like JWTs or session cookies.<br><br>**Why it happens:**<br>• Developers configure wildcard or catch-all CORS rules during testing or integration phases (“just make it work” CORS), and forget to scope them down for production.<br>• Multiple frontends (e.g., admin app, public site, partner dashboard) may consume the same APIs, leading to overgeneralized CORS rules that don't enforce **per-environment origin isolation**.<br>• CORS configuration is often misunderstood — especially around the implications of `credentials: true` combined with wide `Access-Control-Allow-Origin`, which **violates browser security expectations**. |
