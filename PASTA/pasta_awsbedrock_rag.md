...
| 11 | Runtime Controls      | Missing Concurrency/Timeouts <br> **CWE-770 Allocation of Resources Without Limits or Throttling**                | Runtime          | Medium         | Functions without concurrency limits or timeout settings are vulnerable to **Denial of Wallet (DoW)** attacks where attackers invoke long-running functions to incur high cloud costs.                                |
| 12 | Permission Scope      | No SourceArn Restriction <br> **CWE-940: Improper Verification of Source of a Communication Channel**                   | Provision        | High           | If a Lambda permission (like S3 event trigger) lacks `SourceArn` restriction, **any source** can trigger the function, enabling abuse or **unauthorized invocation**.                                                 |
