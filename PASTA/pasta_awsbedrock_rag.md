...
| **#** | **Category**                        | **Vulnerability / CWE / Misconfiguration**                                                                  | **Phase**            | **Risk**           | **Details / Context**                                                                             |
| ----- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------- | ------------------ | ------------------------------------------------------------------------------------------------- |
| 1 | **Frontend Exposure** | **CVE-2023-27494** <br> **CWE-79: Improper Neutralization of Input During Web Page Generation (XSS)** | Runtime | **High**   | Versions 0.63.0–0.80.0 of Streamlit are vulnerable to reflected XSS. A crafted malicious URL can inject JavaScript payloads into the frontend rendered via ECS, leading to browser-side exploitation. This could result in session hijacking, token theft, or prompt manipulation in the LLM chat app, even though Bedrock itself is not directly vulnerable. |
| 2 | **Path Traversal Risk** | **CVE-2022-35918** <br> **CWE-22: Improper Limitation of a Pathname to a Restricted Directory (Path Traversal)** | **Runtime** | **High**   | Vulnerable versions of Streamlit (<1.11.1) using custom components allow crafted URLs to trigger path traversal attacks. In your ECS Docker setup, an attacker could access container-internal files such as server logs, environment files, or even app data, leading to leakage of session info or tokens passed to downstream services like Bedrock. |
| 3    | **ECS Configuration Hygiene**  | **ECS Cluster Without Active Services** ,**CWE-668: Exposure of Internal Resource**               | **Provision**    | **Medium**            | Idle clusters with legacy task definitions or open IAM policies can allow rogue task deployments. This can be abused to perform lateral movement, internal scanning, or secret exfiltration if task roles are over-permissioned. |
| 4    | ECS Orchestration Drift   | **ECS Service Without Running Tasks**,**CWE-668: Exposure of Internal Resource**                   | **Runtime**   | **Medium**        | ECS service exists but runs no tasks. Idle service with legacy permissions or task definitions can be hijacked by attackers to run privileged containers, access internal APIs (S3, OpenSearch), or perform lateral movement. |
