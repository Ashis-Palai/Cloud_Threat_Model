...
| 13    | **Overprivileged Task Execution Role**         | **CWE-250: Execution with Unnecessary Privileges**                                                                                                                                                                             | Runtime         | **High**   | The ECS task execution role (and task role) often has wildcards or full administrative policies (`AdministratorAccess`, `ecs:*`, `iam:*`). If compromised, the attacker can escalate within the AWS environment, manipulate other services, or exfiltrate secrets from AWS APIs. Follow the principle of least privilege for all IAM roles.                                                                                                                                                                                                                                                                                                                                                                 |
| 14 | **Ephemeral Storage Encryption / KMS Misconfiguration** | **CWE-311: Missing Encryption of Sensitive Data<br>- CWE-312: Cleartext Storage of Sensitive Information**<br>- Misconfiguration: Missing or incorrect Customer-Managed KMS (CMK) assignment / over-permissive or insufficient KMS key policy (e.g., failing to grant the Fargate service principal GenerateDataKey* permissions) | Deployment + Runtime | **High** | Fargate tasks use ephemeral storage (20 GiB by default on platform version 1.4.0+, configurable up to 200 GiB via ephemeralStorage). If ephemeral storage is not encrypted with a customer-managed KMS key (or a CMK is assigned but the key policy lacks the required Fargate service principal/grants), sensitive runtime artifacts (temporary files, caches, uploaded files, extracted secrets, swap/tmp data, credentials written to disk) can be stored in cleartext at rest — increasing the risk of data exposure, failing compliance requirements, and enabling post-compromise data disclosure. Misconfigurations to watch for include: running tasks on platform versions or legacy tasks launched before default AES-256 encryption dates, not configuring the cluster’s managedStorageConfiguration.fargateEphemeralStorageKmsKeyId, or using overly-broad KMS key policies that grant unnecessary principals decrypt access (or conversely, missing grants that cause provisioning failures). Recommended mitigations: enforce platform ≥1.4.0, assign and enforce a CMK with least-privilege KMS policy (granting the Fargate service principal necessary GenerateDataKey* permissions), audit KMS usage via CloudTrail, and ensure sensitive data is not written to ephemeral storage when possible. 
